...
class LibraryBuilder:
    ...
    def __init__(self, config: Config | None = None, demo_mode: bool = False):
    ...
    async def build(
        ...
        if categories:
            ...
            target_categories = {
                name: cat for name, cat in STRATEGY_TAXONOMY.items() if name in categories
            }
        else:
        ...
        for priority in ["high", "medium", "low"]:
            ...
            priority_cats = {
                name: cat for name, cat in target_categories.items() if cat.priority == priority
            }
    ...
    async def _build_category(
        self, category_name: str, category_config, min_sharpe: float, output_dir: Path
    ):
        ...
        pipeline = AutonomousPipeline(config=self.config, demo_mode=self.demo_mode)
        ...
        for query in category_config.queries:
            ...
            if strategies_built >= strategies_needed:
                console.print(
                    f"[green]✓ Category target reached ({strategies_built}/{strategies_needed})[/green]"
                )
                break
            ...
            for i in range(attempts_per_query):
                ...
                self.coverage.update(
                    category=category_name, success=success, sharpe=sharpe if success else 0.0
                )
    ...
    async def _generate_one_strategy(
        ...
        try:
            ...
            if not validation["valid"]:
                # Try to fix
                strategy = await pipeline._apply_learned_fixes(
                    strategy, validation.get("errors", [])
                )
    ...
    def _save_strategy_to_library(
        self, strategy: dict, paper: dict, result: dict, category: str, output_dir: Path
    ):
...
"""
    ...
    def _display_build_plan(
        self, comprehensive: bool, max_hours: int, categories: list[str] | None
    ):
        ...
        if categories:
            ...
            for cat in categories:
                if cat in STRATEGY_TAXONOMY:
                    ...
                    console.print(
                        f"  • {cat}: {config.min_strategies} strategies ({config.priority} priority)"
                    )
        else:
            console.print("\n[bold]Building all categories:[/bold]")
            for name, config in STRATEGY_TAXONOMY.items():
                console.print(
                    f"  • {name}: {config.min_strategies} strategies ({config.priority} priority)"
                )
    ...
    async def export(self, format: str = "zip", output_file: Path | None = None):
        ...
        if format == "zip":
            ...
            shutil.make_archive(str(output_file.with_suffix("")), "zip", library_dir)
