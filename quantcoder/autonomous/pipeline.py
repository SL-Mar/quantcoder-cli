...
from quantcoder.autonomous.database import GeneratedStrategy, LearningDatabase
...
class AutonomousPipeline:
    ...
    def _init_mcp_client(self):
        ...
        try:
            ...
            if qc_api_key and qc_user_id:
                ...
                console.print(
                    "[yellow]⚠ QC_API_KEY/QC_USER_ID not set - backtesting disabled[/yellow]"
                )
        except Exception as e:
        ...
        while self.running and iteration < max_iterations:
            ...
            try:
                ...
                success = await self._run_iteration(
                    query=query, iteration=iteration, min_sharpe=min_sharpe, output_dir=output_dir
                )
    ...
    async def _run_iteration(
        self, query: str, iteration: int, min_sharpe: float, output_dir: Path
    ) -> bool:
        ...
        if not validation_result["valid"]:
            console.print(
                f"[yellow]⚠ Validation errors found ({validation_result['error_count']})[/yellow]"
            )
        ...
        # Update stats
        self.stats.avg_sharpe = (self.stats.avg_sharpe * self.stats.total_attempts + sharpe) / (
            self.stats.total_attempts + 1
        )
    ...
    async def _fetch_from_arxiv(self, query: str, limit: int) -> list[dict]:
        ...
        try:
            ...
            response = await asyncio.get_event_loop().run_in_executor(
                None, lambda: requests.get(base_url, params=params, timeout=15)
            )
    ...
    async def _fetch_from_crossref(self, query: str, limit: int) -> list[dict]:
        ...
        try:
            ...
            headers = {"User-Agent": "QuantCoder/2.0 (mailto:quantcoder@example.com)"}

            response = await asyncio.get_event_loop().run_in_executor(
                None, lambda: requests.get(api_url, params=params, headers=headers, timeout=15)
            )
            response.raise_for_status()
            data = response.json()
    ...
    async def _generate_strategy(
        self, paper: dict, enhanced_prompts: dict[str, str]
    ) -> dict | None:
...
Create a complete, compilable algorithm with proper Universe selection, Alpha model, and Risk management."""
    ...
    async def _validate_and_learn(self, strategy: dict, iteration: int) -> dict:
        ...
        if self.mcp_client and not errors:
            try:
                mcp_result = await self.mcp_client.validate_code(
                    code=main_code, files={k: v for k, v in code_files.items() if k != "Main.py"}
                )
    ...
    def _validate_quantconnect_code(self, main_code: str, code_files: dict[str, str]) -> list[str]:
        ...
        common_mistakes = [
            ...
            (
                r"\bself\.Securities\[",
                r"\bself\.AddEquity\b|\bself\.AddCrypto\b|\bself\.SetUniverseSelection\b",
    ...
    async def _apply_learned_fixes(self, strategy: dict, errors: list[str]) -> dict:
    ...
    async def _backtest(self, strategy: dict) -> dict:
        ...
        try:
            main_code = strategy.get("code", "") or strategy.get("code_files", {}).get(
                "Main.py", ""
            )
            code_files = strategy.get("code_files", {})
            ...
            if not result.get("success"):
                console.print(
                    f"[red]✗ Backtest failed: {result.get('error', 'Unknown error')}[/red]"
                )
                return {
                    "sharpe_ratio": 0.0,
                    "max_drawdown": 0.0,
            ...
            total_return = self._parse_stat(
                result.get("total_return") or stats.get("Total Net Profit", 0)
            )
            ...
            return {"sharpe_ratio": 0.0, "max_drawdown": 0.0, "total_return": 0.0, "error": str(e)}
    ...
    def _store_strategy(
        self, strategy: dict, paper: dict, backtest_result: dict, success: bool, output_dir: Path
    ):
...
"""
    ...
    async def _should_continue(self, iteration: int, max_iterations: int) -> bool:
        ...
        if iteration % 10 == 0 and iteration < max_iterations:
            response = Prompt.ask(
                "\nContinue autonomous mode?", choices=["y", "n", "p"], default="y"
            )
    ...
    async def _generate_final_report(self):
        ...
        if common_errors:
            ...
            for i, error in enumerate(common_errors, 1):
                fix_rate = (
                    (error["fixed_count"] / error["count"] * 100) if error["count"] > 0 else 0
                )
                console.print(
                    f"  {i}. {error['error_type']}: {error['count']} occurrences ({fix_rate:.0f}% fixed)"
                )
