...
"""
    ...
    def _build_error_section(self, common_errors: list[dict], successful_fixes: list[dict]) -> str:
    ...
    def _build_performance_section(self, perf_stats: dict, strategy_type: str) -> str:
        ...
        if avg_sharpe >= 1.0:
            ...
            section += "Target: Aim for Sharpe > 1.0 (improve on historical average)\n"

        return section

    def _find_fix_for_error(self, error_type: str, successful_fixes: list[dict]) -> str:
    ...
    def _extract_common_patterns(self, strategies: list[dict]) -> list[dict]:
        ...
        risk_mgmt_count = sum(
            1 for s in strategies if "risk" in str(s.get("code_files", {})).lower()
        )
        ...
        universe_count = sum(
            1 for s in strategies if "universe" in str(s.get("code_files", {})).lower()
        )
        if universe_count >= 2:
            patterns.append(
                {
        ...
        alpha_count = sum(
            1
            for s in strategies
            if "alpha" in str(s.get("code_files", {})).lower()
            or "insight" in str(s.get("code_files", {})).lower()
        )
        if alpha_count >= 2:
            patterns.append(
                {
        ...
        avg_sharpe = sum(s["sharpe_ratio"] for s in strategies if s.get("sharpe_ratio")) / len(
            strategies
        )
        patterns.append(
            {
        ...
        avg_refinements = sum(s.get("refinement_attempts", 0) for s in strategies) / len(strategies)
    ...
    def get_enhanced_prompts_for_agents(self, strategy_type: str = None) -> dict[str, str]:
        ...
        for agent_type, base_prompt in base_prompts.items():
            enhanced[agent_type] = self.inject_learnings(base_prompt, strategy_type=strategy_type)
