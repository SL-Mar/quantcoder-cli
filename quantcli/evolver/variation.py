...
class VariationGenerator:
    ...
    def generate_variations(
        ...
        for i in range(num_variations):
            ...
            if len(parents) >= 2 and random.random() > self.config.mutation_rate:
                ...
                code, description = self._crossover(parent1, parent2)
                parent_ids = [parent1.id, parent2.id]
            else:
                # Mutation
                parent = random.choice(parents)
                strategy = random.choice(self.mutation_strategies)
                code, description = self._mutate(parent, strategy)
    ...
    def _mutate(self, parent: Variant, strategy: str) -> Tuple[Optional[str], str]:
        """
        ...
        try:
            ...
            description = f"Mutation ({strategy}): {self._extract_description(content)}"

            return code, description

        except openai.OpenAIError as e:
            self.logger.error(f"OpenAI API error during mutation: {e}")
            return None, f"Failed: {e}"

    def _crossover(self, parent1: Variant, parent2: Variant) -> Tuple[Optional[str], str]:
        """
        ...
        try:
            ...
            content = response.choices[0].message['content'].strip()
            code = self._extract_code(content)
            description = f"Crossover ({parent1.id} × {parent2.id}): {self._extract_description(content)}"

            return code, description

        except openai.OpenAIError as e:
            self.logger.error(f"OpenAI API error during crossover: {e}")
            return None, f"Failed: {e}"
...
"""
    ...
    def generate_initial_variations(
        self,
        baseline_code: str,
        num_variations: int
    ) -> List[Tuple[str, str, List[str]]]:
        ...
        for i in range(num_variations):
            ...
            code, description = self._mutate(baseline_variant, strategy)
