...
class EvolutionEngine:
    ...
    def evolve(
        ...
        try:
            ...
            while True:
                ...
                # Generate variations
                variants = self._generate_generation(generation)
                ...
                # Evaluate variants
                self._evaluate_variants(variants)
    ...
    def _generate_generation(self, generation: int) -> list[Variant]:
        """Generate variants for a new generation."""

        if generation == 1:
            # First generation: vary from baseline
            raw_variations = self.variation_generator.generate_initial_variations(
            ...
            raw_variations = self.variation_generator.generate_variations(
                parents,
                self.config.variants_per_generation,
                generation
            )
    ...
    def _evaluate_variants(self, variants: list[Variant]):
        ...
        for variant in variants:
            ...
            result = self.evaluator.evaluate(variant.code, variant.id)
...
def create_evolution_engine(
    ...
    **kwargs
    ...
    Example:
        ...
        result = engine.evolve(baseline_code)
    """
    config = EvolutionConfig(
