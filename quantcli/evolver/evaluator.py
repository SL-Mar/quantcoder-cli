...
class QCEvaluator:
    ...
    def _api_request(
        ...
        try:
            if method == "GET":
                ...
                response = requests.post(url, auth=self._get_auth(), json=data, timeout=30)
            else:
    ...
    def create_project(self, name: str) -> Optional[int]:
        ...
        result = self._api_request("POST", "projects/create", data)
    ...
    def update_project_code(self, project_id: int, code: str, filename: str = "main.py") -> bool:
        ...
        result = self._api_request("POST", "files/update", data)
        if result and result.get("success"):
    ...
    def compile_project(self, project_id: int) -> Optional[str]:
        ...
        result = self._api_request("POST", "compile/create", data)
        if result and result.get("success"):
            ...
            while state == "InQueue" and waited < max_wait:
                time.sleep(2)
                waited += 2
                status = self._api_request("GET", f"compile/read?projectId={project_id}&compileId={compile_id}")
                if status:
    ...
    def run_backtest(self, project_id: int, compile_id: str, name: str) -> Optional[str]:
        ...
        result = self._api_request("POST", "backtests/create", data)
        if result and result.get("success"):
    ...
    def wait_for_backtest(self, project_id: int, backtest_id: str, timeout: int = 300) -> Optional[dict]:
    ...
    def evaluate(self, code: str, variant_id: str) -> Optional[BacktestResult]:
        """
        ...
        # Step 1: Update code
        if not self.update_project_code(project_id, code):
            return None
        ...
        backtest_id = self.run_backtest(project_id, compile_id, backtest_name)
        if not backtest_id:
            return None

        # Step 4: Wait and get results
        backtest_data = self.wait_for_backtest(project_id, backtest_id)
    ...
    def evaluate_batch(self, variants: list) -> Dict[str, Optional[BacktestResult]]:
        """
        ...
        for variant_id, code in variants:
            result = self.evaluate(code, variant_id)
